...
def dashboard(request):
    ...
    print("current_month_due",current_month_due)
    print("current_month_due_adai",current_month_due_adai)
    previous_month_due = previous_month_due - previous_month_due_adai

    # Allow due amounts to be negative (no max constraint)
    # total_due, current_month_due, and previous_month_due can be negative
    ...
    context = {
        ...
        "due": {
            "current_month": (current_month_sale - current_month_commission)-(current_month_advance_adai+current_month_due_adai+current_month_payment_received),
            "all_time": total_due,
        },
        "payment_received": {
        ...
        "mot_adai": {
            ...
            "all_time": total_advance+total_due_adai+total_payment_received,
        },
    }
    ...
    try:
        ...
        for customer in customers:
            ...
            excel_data.append({
                ...
                'Phone': customer.phone,
                'NID': customer.nid,
                'Due': float(customer.due or 0),
                'Advance': float(customer.Advance or 0),
    ...
    if request.method == 'POST':  # Only allow POST requests
        try:
            with transaction.atomic():
                ...
                # Update customer
                if customer:
                    # Calculate how much advance was used in this sale
                    # Net amount after discount (before less, as less is separate)
                    net_after_discount = (sale.total_price or 0) - (sale.discount or 0)
                    # Calculate what the due would have been without advance usage
                    # If payment_received + sale.due < net_after_discount, then advance was used
                    total_covered = (sale.payment_received or 0) + (sale.due or 0)
                    advance_used = 0
                    if total_covered < net_after_discount:
                        # Advance was used to cover the difference
                        advance_used = net_after_discount - total_covered
                    
                    # When deleting a sale, we need to:
                    # 1. Restore the advance that was used during sale creation (if any)
                    # 2. Subtract the original paid amount from Paid
                    # Note: original_due should NOT be added to advance - it's just a debt that will be removed
                    customer.Advance += advance_used
                ...
                # Calculate initial due before deleting the sale
                # Initial due is the difference between customer.due and calculated due from all sales/adai
                initial_due = 0
                if customer:
                    from django.db.models import Sum
                    # Calculate current due from all sales and adai (including the sale we're about to delete)
                    total_sale_due_before = Sale.objects.filter(customer=customer).aggregate(total=Sum('due'))['total'] or 0
                    total_adai_due_before = Adai.objects.filter(customer=customer).aggregate(total=Sum('due'))['total'] or 0
                    calculated_due_before = total_sale_due_before - total_adai_due_before
                    # Initial due is the difference (this preserves any manually set initial due)
                    initial_due = customer.due - calculated_due_before
                ...
                # Recalculate the customer's Due from all remaining records
                # Note: We preserve customer.Advance as manually adjusted above, don't overwrite it
                if customer:
                    from django.db.models import Sum
                    ...
                    # Recalculate due from remaining sales/adai and add back the initial due
                    customer.due = (total_sale_due - total_adai_due) + initial_due
                    if customer.due < 0:
                        customer.due = 0
                    # Advance is already restored above, don't overwrite it
                    customer.save()
...
def download_cash_memo(request, sale_id):
    try:
        ...
        # Calculate net bill
        who_less = 0
        total_less_per_unit = 0  # sum of (lessPerUnit * quantity) for all products
        for i in range(len(sale.products)):
            item = sale.products[i]
            if item.get('lessPerUnit', 0) > 0:
                who_less += 1
            less_per_unit = float(item.get('lessPerUnit', 0) or 0)
            qty = float(item.get('quantity', 0) or 0)
            total_less_per_unit += less_per_unit * qty
            print(total_less_per_unit)

        if who_less > 0:
            net_bill = (sale.total_price or 0) - (sale.discount or 0)-(sale.less-total_less_per_unit)
            mot_bill=sale.total_price
            print(mot_bill,net_bill,sale.less)
        else:
        ...
        context = {
            ...
            'previous_due': previous_due,
            'customer_advance': customer.Advance or 0,
            'less': sale.less-total_less_per_unit,
        }
        print(context)
...
def delete_adai(request, adai_id):
    if request.method == 'POST':
        try:
            ...
            # Subtract the advance amount that was added by this Adai record
            ...
            if customer:
                ...
                if customer.due < 0:
                    customer.due = 0
                # Advance is already adjusted above (subtracted the deleted Adai's advance), don't overwrite it
                customer.save()
...
def filter_customer_sales(request):
    if request.method == 'POST':
        ...
        print(customer.Advance)
        if customer.due>0:
            cutomer_balance=customer.due

        elif customer.Advance>0:
            cutomer_balance=-(customer.Advance)
        else:
            cutomer_balance=0
        filtered_sales = []
        for sale in sales_records:
            # Sum all product less per unit: lessPerUnit * quantity for each line
            ...
            if sale.products:
                for item in sale.products:
                    ...
                    qty = float(item.get('quantity', 0) or 0)
                    total_less_per_unit += less_per_unit * qty
            sale_less = float(sale.less or 0)
            total_price = (float(sale.total_price or 0) + float(sale.discount or 0)) +total_less_per_unit
            filtered_sales.append({
                ...
                'Customer_due': customer.due,
                'less': sale.less,
                'net_sale': sale.total_price-sale.less+total_less_per_unit,
                'invoice_number': sale.invoice_number or 'N/A',
            ...
            try:
                ...
                for sale in sales:
                    ...
                    transactions.append({
                        "date": sale.date,
                        "particulars": "Sale",
                        "debit": sale.total_price - sale.discount,
                        "credit": 0,
                ...
                # --- Add Manual Advance Adjustment if needed ---
                # Calculate advance from all adai records (all time)
                all_adai = Adai.objects.filter(customer=customer)
                calculated_advance_from_adai = sum(a.advance for a in all_adai)
                
                # Calculate advance used in sales (all time)
                # When a sale uses advance, it reduces customer.Advance
                # We need to reconstruct the initial advance by adding back advance used
                all_sales_for_advance = Sale.objects.filter(customer=customer)
                advance_used_in_sales = 0
                for sale in all_sales_for_advance:
                    # Calculate net amount after discount
                    net_after_discount = (sale.total_price or 0) - (sale.discount or 0)
                    # Calculate what was covered by payment and due
                    total_covered = (sale.payment_received or 0) + (sale.due or 0)
                    # If total_covered < net_after_discount, advance was used
                    if total_covered < net_after_discount:
                        advance_used = net_after_discount - total_covered
                        advance_used_in_sales += advance_used
                
                # Current advance (after all sales have used some)
                current_advance = float(customer.Advance or 0)
                
                # Initial manual advance = Current advance + Advance used in sales - Calculated advance from Adai
                # This gives us the initial manually set advance
                initial_manual_advance = current_advance + advance_used_in_sales - calculated_advance_from_adai
                
                # Manual adjustment is the difference between initial manual advance and calculated advance
                manual_advance_adjustment = initial_manual_advance - calculated_advance_from_adai
                
                if manual_advance_adjustment != 0:
                    transactions.append({
                        "date": None,
                        "particulars": "Manual Advance Adjustment",
                        "debit": abs(manual_advance_adjustment) if manual_advance_adjustment < 0 else 0,
                        "credit": manual_advance_adjustment if manual_advance_adjustment > 0 else 0,
                        "desc": f"Initial manual advance: {initial_manual_advance:.2f} (Current: {current_advance:.2f} + Used in sales: {advance_used_in_sales:.2f} - From Adai: {calculated_advance_from_adai:.2f})",
                        "invoice_number": "INIT-ADV"
                    })
                ...
                calculated_due = sum(sale.due for sale in all_sales)
                calculated_due -= sum(adai.due for adai in all_adai)
                actual_due = float(customer.due or 0)
                manual_adjustment = actual_due - calculated_due
                if manual_adjustment != 0:
                    transactions.append({
                        ...
                        "desc": f"Manually set due: {actual_due:.2f} vs calculated: {calculated_due:.2f}",
...
def download_salesman_report(request):
    ...
    try:
        ...
        total_collection = sum(sale.payment_received for sale in sales) + sum(adai.due for adai in adai_records) +sum(adai.advance for adai in adai_records) # Only for selected month
        total_commission = total_collection*.008 # Only for selected month
        total_due = sum(sale.due for sale in all_sales)- total_collection # All time due
...
def export_salesman_report_excel(request):
    """Export salesman report (sales + adai for selected month/year) to Excel."""
    salesman_name = request.GET.get('salesman')
    month = request.GET.get('month')
    year = request.GET.get('year')
    if not salesman_name or not month or not year:
        return HttpResponse("Missing parameters: salesman, month, year", status=400)
    try:
        month = int(month)
        year = int(year)
        salesman = Salesman.objects.get(name=salesman_name)
    except (ValueError, Salesman.DoesNotExist):
        return HttpResponse("Invalid salesman or date", status=404)
    sales = Sale.objects.filter(salesman=salesman, date__year=year, date__month=month)
    adai_records = Adai.objects.filter(salesman=salesman, date__year=year, date__month=month)
    sales_data = []
    for sale in sales:
        sales_data.append({
    ...
    response['Content-Disposition'] = f'attachment; filename="salesman_report_{safe_name}_{year}_{month:02d}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx"'
    return response
